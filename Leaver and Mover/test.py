...
CASE 
    ...
    * 
FROM systemschema.hr_data) as m on t.employee_id = m.employee_id where m.date<=CURRENT_DATE + INTERVAL '5 days'""")
    rows = cur.fetchall()
...
FROM systemschema.hr_data) as m on t.employee_id = m.employee_id where m.date<=CURRENT_DATE + INTERVAL '5 days' """)
    ...
    for current_manager in data2:
        ...
        <body>
        <p>Dear Manager,</p>
        <p>Please find the below entries that are pending with employee who are direct/indirect report to you.</p>
        <a href="http://127.0.0.1:5000/">click here and update the record under HR module if you are not direct repotee</a>
        {table_html}
...
def get_employee():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute('''select t.*, m.po_count, m.invoice_count, m.req_count from systemschema.hr_data t inner join ( 
select count(case when type = 'Purchase Order' then doc_id end) as PO_Count, count(case when type = 'Invoice' then doc_id end) as Invoice_Count, 
count(case when type = 'Requisition' then doc_id end) as Req_Count, employee_id from systemschema.documents group by employee_id) m on t.employee_id = m.employee_id''')
    rows = cur.fetchall()
    columns = [desc[0] for desc in cur.description]
    cur.close()
    conn.close()
...
def record_count():
    conn = get_db_connection()
    cur = conn.cursor()
    cur.execute("""select count(distinct t.employee_id) from systemschema.hr_data t inner join systemschema.documents m on t.employee_id = m.employee_id where t.case_status = 'leaver' """)
    moving_count = cur.fetchone()[0]
    cur.execute("select count(distinct t.employee_id) from systemschema.hr_data t inner join systemschema.documents m on t.employee_id = m.employee_id where t.case_status = 'mover' ")
...
def update_employee(employee_id):
    ...
    conn = get_db_connection()
    cur = conn.cursor()
    employee_name = None if data['employee_name'] == "" else data['employee_name']
    line_manager_email = None if data['line_manager_email'] == "" else data['line_manager_email']
    line_manager2 = None if data['line_manager2'] == "" else data['line_manager2']
    line_manager3 = None if data['line_manager3'] == "" else data['line_manager3']
    line_manager4 = None if data['line_manager4'] == "" else data['line_manager4']
    cur.execute("""
        ...
        WHERE id = %s
    """, (
        employee_name,  line_manager_email, line_manager2,
        line_manager3, line_manager4, data['case_status'], employee_id
    ))
    conn.commit()
    cur.close()
    conn.close()
